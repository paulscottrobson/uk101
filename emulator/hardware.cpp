...
#include "fabgl.h"
void writeCharacter(int,int,int);
...
void HWSync(void) {
	...
	int scanCode = Keyboard.getNextScancode(0);
	if (scanCode > 0) {
		if (scanCode == 0xE0) {
			shift = 0x80;
		} 
...
void HWWriteDisplay(WORD16 address,BYTE8 data) {	
	address = address - 0xD085;
	if (address < 0) return;
	int x1 = address & 0x1F;
	int y1 = address >> 5;
	if (x1 < 24 and y1 < 24) {
		writeCharacter(x1,y1,data);
	}
}
...
static const BYTE8 keyboardMap[] = {
		...
		0x00,0x14,0x00,0x00,0x00,0x12,0x59,0x00		// . CTL . . . LShift RShift .
};
